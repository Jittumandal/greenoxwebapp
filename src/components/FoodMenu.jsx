...
const menuItems = {
  Special: [
    {
      id: 1,
      name: "Classic Sandwich ( Cheese & Paneer ) Corn Sandwich",
...
const FoodMenu = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8" id="target-section">
      <div className="mb-8 flex flex-col items-stretch sm:flex-row sm:items-center sm:justify-between">
        <div className="mb-8">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-4">
          {Object.keys(menuItems).map((tab) => (
            <button
              ...
              className={`relative w-full rounded-md px-4 py-3 text-base font-medium transition-colors sm:w-auto sm:py-2 ${
                activeTab === tab
      ...
      <div className="mb-12 grid grid-cols-1 gap-8" id="target-section">
        {activeContent.map((item) => (
          <div key={item.id} className="flex flex-col gap-8 md:flex-row">
            ...
            <Link
              to={`/menu/${item.id}`}
              className="group flex flex-1 rounded-lg p-4 transition-shadow hover:shadow-lg"
            >
              <div className="flex gap-4">
                ...
                <div className="flex flex-col justify-start">
                  <h3 className="line-clamp-1 text-xl font-semibold text-gray-800 group-hover:text-orange-500">
                    {item.name}
                  </h3>
